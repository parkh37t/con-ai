...
| # | 에이전트 | 롤 | 핵심 스킬·규범 | 도구 | 산출 |
|---|----------|----|--------------|------|------|
| 1 | **wiley-intake** | 인테이크·리서치 | ia-from-url · screen-inventory · benchmark-painpoint · 01·04 | Read·Grep·Glob·Bash·Web | IA 구조도·화면 카탈로그·벤치 리포트 |
| 2 | **wiley-extract** | 산출물 추출 | 01_intake-router · ScreenSpec 스키마 | Read·Write·Bash·Grep·WebFetch | ScreenSpec JSON |
| 3 | **wiley-screen** | 화면 생산 ★ | screen-spec · 07(8단·마커·골든·5원칙) | Read·Write·Edit·Bash·Grep·Glob | 2단 HTML 화면설계서 |
| 4 | **wiley-diagram** | 도식·비주얼 | 06_diagramming(Wireflow·상태도·시퀀스·매트릭스) | Read·Edit·Write·Bash | 도식 강화·비주얼 |
| 5 | **wiley-verify** | 검증 게이트 | 07§G·H·C · audit/patch 도구 | Read·Bash·Grep·Glob·Edit | 검증 리포트·자동정정 |
| 6 | **wiley-render** | 렌더 타깃 | 02_mcp-map(Figma·Express·Canva) | Read·Bash + MCP(ToolSearch) | Figma/Express/Canva·이미지 |
...
[입력]  URL · PPTX · Figma · 기획서
   ...
   ▼  ⑥ wiley-render     (선택) Figma/Express/Canva 렌더 타깃 변환
[출고]  화면설계서(SSOT) + 렌더 산출물
```

- **역할 경계 불변식**: intake/verify는 화면을 만들지 않고(읽기·검사), screen만 백지→생산, diagram은 기존 강화, render는 변환. 경계를 넘지 않는다.
