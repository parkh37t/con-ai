 ...
 *
 * 시각 언어는 화면설계서(packages/renderer/src/styles.ts)와 같은 것을 쓴다:
 * 흰 바탕·검정 위주 텍스트, 얇은 테두리 카드, 카드 좌상단에 걸친 검은 사각 번호 배지,
 * 강조는 요소 배지와 같은 파랑(#1d6ef5). 그림자·그라데이션·이모지는 쓰지 않는다.
 *
  ...
  return (
    <div className="main-page">
      ...
      <section className="main-hero">
        ...
        <p className="main-sub">목업과 우측 설명을 같은 명세에서 만들고, 디자이너·퍼블리셔·개발자·고객이 같은 화면에서 검토합니다.</p>
      ...
      <section className="main-section" aria-labelledby="main-steps-title">
      ...
      <section className="main-section" aria-labelledby="main-recent-title">
        ...
        {recent.length === 0 ? (
          <>
            <p className="main-note">아직 만든 설계서가 없습니다. 「설계서 만들기」 에서 문장 한 줄이면 첫 설계서가 나옵니다.</p>
            <div className="main-cards">
              <a className="main-card main-card-sample" data-testid="main-sample-card" href={hrefTo('references')}>
                <strong>골든 예시 3종 보기</strong>
          ...
          <div className="main-cards">
            {recent.map((d) => (
              <a key={d.revision_id} className="main-card" data-testid="main-recent-card" data-external-id={d.external_id} href={hrefToDesign(d.revision_id)}>
                ...
                <span className="main-card-sub">{d.external_id}</span>
